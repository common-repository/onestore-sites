...
jQuery( document ).ready( function( $ ){
    ...
    var OneStore_Modal_Site = function( $item, data ){
        var m = this;
        var steps;
        steps = {
            modal: null,
            ...
            _open: function(){
                ...
                that.item.on('click', '.cs-open-modal, .theme-screenshot', function (e) {
                    e.preventDefault();
                    $('body').addClass('onestore-sites-show-modal');
                    if (that.owl) {
                    ...
                    if ( that.data.pro && ! OneStore_Sites.license_valid ) {
                        that.disable_button( 'start' );
                        that.buttons.start.addClass( 'pro-only' );
                        that.buttons.start.find( '.cs-btn-circle-text' ).text( OneStore_Sites.pro_text );
                    }
                ...
                _.each( that.data.manual_plugins, function( plugin_name, plugin_file ){
                    ...
                    if ( ! _.isUndefined( that.recommend_plugins[ plugin_file ] ) ) {
                        if ( ! that.is_installed( plugin_file ) ) {
                            if (_.isUndefined(OneStore_Sites.installed_plugins[plugin_file])) {
            ...
            init: function(){
                ...
                that.modal.on( 'click', '.cs-back-to-list', function( e  ) {
                    e.preventDefault();
                    $( 'body' ).removeClass( 'onestore-sites-show-modal' );
            ...
            is_activated: function( plugin_slug ){
                return _.isUndefined( OneStore_Sites.activated_plugins[ plugin_slug ] ) ? false : true;
            },

            is_installed: function( plugin_slug ){
                return _.isUndefined( OneStore_Sites.installed_plugins[ plugin_slug ] ) ? false : true;
            },
            ...
            _do_start_import: function(){
                ...
                that.buttons.start.on( 'click', function( e  ) {
                    ...
                    if ( ! that.doing ) {
                        ...
                        that.doing = true;
                        that.current_builder = $( '#onestore-sites-filter-cat a.current' ).eq(0).attr( 'data-slug' ) || '';
                        ...
                        $.ajax({
                            ...
                            success: function (res) {
                                ...
                                if (that.xml_id <= 0) {
                                    ...
                                    that.doing = false;
                                    that.buttons.start.find('.cs-btn-circle-text').text(OneStore_Sites.try_again);
                                } else {
                ...
                var option_completed = function(){
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            _nonce: OneStore_Sites.elementor_clear_cache_nonce
                        }
                    });
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            _nonce: OneStore_Sites.elementor_reset_library_nonce
                        }
                    });
    ...
    var OneStore_Site = {
        ...
        load_sites: function ( cb ) {
            ...
            $( '#onestore-sites-listing-wrapper' ).hide();
            that.filter_data = that.get_filter_data();
            that.filter_data['_t'] = new Date().getTime();
            that.xhr = $.ajax( {
                url: OneStore_Sites.api_url,
                data: that.filter_data,
                type: 'GET',
                success: function( res ){
                    that.data = res;
                    $( '#onestore-sites-filter-count' ).text( res.total );
                    that.render_items();
        ...
        render_items: function(){
            ...
            if ( that.data.total <= 0 ) {
                $( '#onestore-sites-listing-wrapper' ).hide();
                $( '#onestore-sites-no-demos' ).show();
                $( 'body' ).addClass('no-results');
            } else {
                $( '#onestore-sites-no-demos' ).hide();
                $( '#onestore-sites-listing-wrapper' ).show();
                $( 'body' ).removeClass('no-results');
            }
            $( '#onestore-sites-listing .theme' ).remove();

            _.each( that.data.posts, function( item ) {
                // console.log( 'item', item );
                var html = template( item );
                var $item_html = $( html );
                $( '#onestore-sites-listing' ).append( $item_html );
                var s = new OneStore_Modal_Site( $item_html, item );
                s.add_modal();
        ...
        render_categories: function(){
            ...
            _.each( that.data.categories, function( item ){
                ...
                $( '#onestore-sites-filter-cat' ).append( html );
            } );
        },
        ...
        render_tags: function(){
            ...
            _.each( that.data.tags, function( item ){
                var html = '<li><a href="#" data-slug="'+item.slug+'">'+item.name+'</a></li>';
                $( '#onestore-sites-filter-tag' ).append( html );
            } );
        },
        ...
        get_filter_data: function(){
            var that = this;
            var cat = $( '#onestore-sites-filter-cat a.current' ).eq(0).attr( 'data-slug' ) || '';
            var tag = $( '#onestore-sites-filter-tag a.current' ).eq(0).attr( 'data-slug' ) || '';
            var s = $( '#onestore-sites-search-input' ).val();
            if ( cat === 'all' ) {
                cat = '';
            }
        ...
        filter: function(){
            var that = this;
            $( document ).on( 'click', '#onestore-sites-filter-cat a', function( e ){
                e.preventDefault();
                if ( ! $( this ).hasClass( 'current' ) ) {
                    $('#onestore-sites-filter-cat a').removeClass('current');
                    $('#onestore-sites-filter-tag a').removeClass('current');
            ...
            $( document ).on( 'click', '#onestore-sites-filter-tag a', function( e ){
                e.preventDefault();
                if ( ! $( this ).hasClass( 'current' ) ) {
                    $('#onestore-sites-filter-tag a').removeClass('current');
                    $(this).addClass('current');
            ...
            // Search demo
            $( document ).on( 'change keyup', '#onestore-sites-search-input', function(){
                $( '#onestore-sites-filter-cat a' ).removeClass( 'current' );
                $( '#onestore-sites-filter-tag a' ).removeClass( 'current' );
                that.skip_render_filter = true;
                that.filter_data = that.get_filter_data();
        ...
        view_details: function(){
            ...
            //$( 'body' ).addClass( 'onestore-sites-show-modal' );
            /*
            $( document ).on( 'click', '#onestore-sites-listing .theme', function( e ){
                e.preventDefault();

            } );

            $( '.cs-modal' ).each( function(){
                var s = new OneStore_Modal_Site( $( this ) );
                s.init();
            } );
            */
    ...
    var OneStore_Site_Preview = function(){
        var preview = {
            el: $( '#onestore-site-preview' ),
            previewing: '',
            init: function(){
                var that = this;
                // open view
                $( document ).on( 'click', '.cs-open-preview', function( e ) {
                    ...
                    if( ! _.isUndefined( OneStore_Site.data.posts[ slug ] )  ) {
                        var data = OneStore_Site.data.posts[ slug ];
                        that.previewing = data.slug;
                        $( '#onestore-site-preview' ).removeClass( 'cs-iframe-loaded' );
                ...
                $( document ).on( 'click', '.cs-preview-nav', function( e ) {
                    ...
                    var current_demo = $( '#onestore-sites-listing .theme[data-slug="'+that.previewing+'"]' );
                    var $item;
                    if ( action === 'next' ) {
    ...
    OneStore_Site.init();
    OneStore_Site_Preview().init();
    $( '#cs-preview-iframe' ).load( function(){
        $( '#onestore-site-preview' ).addClass( 'cs-iframe-loaded' );
    } );
