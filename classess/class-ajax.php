...
class OneStore_Sites_Ajax {
	...
	function __construct() {
		// Install Plugin
		add_action( 'wp_ajax_cs_install_plugin', array( OneStore_Sites_Plugin::get_instance(), 'ajax' ) );
		// Active Plugin
		add_action( 'wp_ajax_cs_active_plugin', array( OneStore_Sites_Plugin::get_instance(), 'ajax' ) );
	...
	function _get_widgets_export_data() {
		...
		$data = apply_filters( 'onestore_sites_export_widgets_data', $sidebars_widget_instances );
	...
	function ajax_export() {
		...
		$include_plugins = apply_filters(
			'onestore-sites/export_plugins/exclude',
			array(
				'autobar' => 1,
				'onestore-sites' => 1,
				'onestore-sites-api' => 1,
				'onestore-sites-listing' => 1,
				'theme-sniffer' => 1,
				'wp-content-crawler' => 1,
				'query-monitor' => 1,
		...
		$config = apply_filters( 'onestore-sites/export/json', $config );
	...
	function ajax_import_content() {
		...
		$import_ui = new OneStore_Sites_WXR_Import_UI();
		$import_ui->import();
	...
	function ajax_download_files() {
		...
		$placeholder_only = apply_filters( 'onestore_import_placeholder_only', true );

		update_option( 'onestore_import_placeholder_only', $placeholder_only );
		...
		*/
		...
		$return['texts']['post_count'] = sprintf( _n( '%d post (including CPT)', '%d posts (including CPTs)', $return['summary']['post_count'], 'onestore-sites' ), $return['summary']['post_count'] );
		$return['texts']['media_count'] = sprintf( _n( '%d media item', '%d media items', $return['summary']['media_count'], 'onestore-sites' ), $return['summary']['media_count'] );
		$return['texts']['user_count'] = sprintf( _n( '%d user', '%d users', $return['summary']['user_count'], 'onestore-sites' ), $return['summary']['user_count'] );
		$return['texts']['term_count'] = sprintf( _n( '%d term', '%d terms', $return['summary']['term_count'], 'onestore-sites' ), $return['summary']['term_count'] );
		$return['texts']['comment_count'] = sprintf( _n( '%d comment', '%d comments', $return['summary']['comment_count'], 'onestore-sites' ), $return['summary']['comment_count'] );
	...
	function ajax_import_options() {
		...
		if ( $file ) {
			...
			$customize_data = OneStore_Sites_Placeholder::get_instance()->progress_config( $customize_data );
	...
	function _import_options( $options, $re_mapping_posts = false ) {
		...
		if ( $re_mapping_posts ) {
			...
			if ( function_exists( 'wc_get_page_id' ) ) {
				foreach ( array( 'myaccount', 'shop', 'cart', 'checkout', 'view_order', 'terms' ) as $page_name ) {
					$id = wc_get_page_id( $page_name );
					if ( $id > 0 ) { // if this page id already exists.
						$key = 'woocommerce_' . $page_name . '_page_id';
						$page_id = isset( $options[ $key ] ) ? $options[ $key ] : false;
						unset( $options[ $key ] );
	...
	function _import_widgets( $data, $widgets_config = array() ) {
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				$widget = OneStore_Sites_Placeholder::get_instance()->replace_placeholder( $widget );
				...
				if ( isset( $widget_instances[ $base_id ] ) ) {
					...
					$single_widget_instances[] = apply_filters( 'onestore_sites_import_widget_data', $widget, $widget_instances[ $base_id ], $base_id );
